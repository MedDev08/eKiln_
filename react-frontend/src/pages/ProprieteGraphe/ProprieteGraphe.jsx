...
export default function ProprieteGraphe() {
  ...
  { field: 'id_four', headerName: 'Four', width: 200,
     renderCell: (params) => params.row.four?.num_four
   },
  { field: 'V1', headerName: 'V1', width: 200 },
  { field: 'V2', headerName: 'V2', width: 200 },
  ...
  },
  {
    ...
    renderCell: (params) => (
      <>
        <IconButton color="primary" onClick={() =>navigate(`/settings/propriete_graphe/edit/${params.row.id}`)}>
          <EditIcon />
        </IconButton>
 ...
 React.useEffect(() => {
  const fetchZones = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:8000/api/propriete-graphe', {
 ...
 const handleSearch = (event) => {
  ...
  const handleAddClick = () => navigate('/settings/propriete_graphe/add');
  const handleDeleteClick = (id) => { setDeleteId(id); setOpenDialog(true); };

  const handleConfirmDelete = async () => {
    try {
        await axios.delete(`http://localhost:8000/api/propriete-graphe/${deleteId}`, {
        headers: { Authorization: `Bearer ${token}` },
        });
        setZones(prev => prev.filter(z => z.id !== deleteId));
        setFilteredZones(prev => prev.filter(z => z.id !== deleteId));
        setSnackbarMessage("Zone supprimée avec succès");
  ...
  return (
    <SidebarChef initialPath="/settings/propriete_graphe">
      <Box m="30px">
      <Header
        title="Propriétés Graphe"
        subtitle="Créer, modifier et supprimer les zones pour les graphiques"
        />
        <Box
          mt="25px"
          height="55vh"
        ...
        >
          ...
          <Dialog open={openDialog} onClose={handleCancelDelete}>
            <DialogTitle>Supprimer ce Zone de graphe  ?</DialogTitle>
            <DialogActions>
